(function ($) {
    ...
    var methods = {
        ...
        */
         ...
         */
        init: function (sourceControl, statusControl, viewModeControl) {
            ...
            if (viewModeControl.value == "1") {
                methods.setReadOnlyMode(this.find('table')[0]);
            }
        },
        /**
        ...
        /**
         * Метод устанавливает режим только для чтения для контрола
         * @param {type} table
         */
        setReadOnlyMode: function (table) {
            $(table).find('input').prop("disabled", true);
            $(table).find('.btn-add')[0].onclick = {};
        },
        /**
         * Метод снимает режим только для чтения для контрола
         * @param {type} table
         */
        unSetReadOnlyMode: function (table) {
            $(table).find('input[type="text"]').prop("disabled", false);
            $(table).find('.btn-add')[0].onclick = function () {
                var tableContainer = $(table).closest('.TSVContainer')[0];
                var sourceControl = document.getElementById(tableContainer.dataset.sourceControl);
                var statusControl = document.getElementById(tableContainer.dataset.statusControl);
                th3.onclick = function () {
                    methods.addRow($(table).find('tbody')[0], sourceControl, statusControl, "", "", "", "");
                }
